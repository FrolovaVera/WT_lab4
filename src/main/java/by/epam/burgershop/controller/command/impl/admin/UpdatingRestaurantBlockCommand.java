...
/**
 * The type Updating restaurant block command.
 */
public class UpdatingRestaurantBlockCommand implements Command {
    ...
    public CommandResult execute(HttpServletRequest request) {
        ...
        String newBlock = request.getParameter(RequestParameter.UPDATING_RESTAURANT_BLOCK);
        ...
        if (!restaurantValidator.isValidNewBlock(newBlock)) {
            request.setAttribute(RequestAttribute.UPDATING_RESTAURANT_BLOCK_ERROR, BundleKey.PHARMACY_BLOCK_ERROR);
            return new CommandResult(PagePath.UPDATING_RESTAURANT_BLOCK, CommandResult.RoutingType.FORWARD);
        }
        try {
            restaurantService.updateBlock(id, newBlock);
            if (currentPage != 1) {
