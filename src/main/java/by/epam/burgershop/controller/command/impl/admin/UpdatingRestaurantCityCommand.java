...
/**
 * The type Updating restaurant city command.
 */
public class UpdatingRestaurantCityCommand implements Command {
    ...
    public CommandResult execute(HttpServletRequest request) {
        ...
        String newCity = request.getParameter(RequestParameter.UPDATING_RESTAURANT_CITY);
        ...
        if(!restaurantValidator.isValidCityOrStreet(newCity)) {
            request.setAttribute(RequestAttribute.UPDATING_RESTAURANT_CITY_ERROR, BundleKey.PHARMACY_STRING_PARAMETERS_ERROR);
            return new CommandResult(PagePath.UPDATING_RESTAURANT_CITY, CommandResult.RoutingType.FORWARD);
        }
        try {
            restaurantService.updateCity(id, newCity);
            if (currentPage != 1) {
