    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel"><fmt:message key="header.add_restaurant"/></h5>
            ...
            <div class="modal-body">
                <form action="${pageContext.request.contextPath}/controller?command=addition_restaurant" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="number" class="form-control" required value="${requestScope.mapData.get("number")}"
                               placeholder="<fmt:message key="placeholder.number"/>" size="35px"/><br>
                    </div>
                    <div class="form-group">
                        <label class="field"><fmt:message key="label.city"/></label><br>
                        <input type="text" name="city" class="form-control" required value="${requestScope.mapData.get("city")}"
                               placeholder="<fmt:message key="placeholder.city"/>" size="35px"/><br>
                    </div>
                    <div class="form-group">
                        <label class="field"><fmt:message key="label.street"/></label><br>
                        <input type="text" name="street" class="form-control" required value="${requestScope.mapData.get("street")}"
                               placeholder="<fmt:message key="placeholder.street"/>" size="35px"/><br>
                    </div>
                    <div class="form-group">
                        <label class="field"><fmt:message key="label.house"/></label><br>
                        <input type="text" name="house" class="form-control" required value="${requestScope.mapData.get("house")}"
                               placeholder="<fmt:message key="placeholder.house"/>" size="35px"/><br>
                    </div>
                    <div class="form-group">
                        <label class="field"><fmt:message key="label.block"/></label><br>
                        <input type="text" name="block" class="form-control" value="${requestScope.mapData.get("block")}"
                               placeholder="<fmt:message key="placeholder.block"/>" size="35px"/><br>
                    </div>
                    <input type="submit" class="btn btn-primary" value="<fmt:message key="button.input_restaurant"/>" class="button">
                </form>
...
<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal">
    <fmt:message key="modal.buttun_add_restaurant"/>
</button>
...
<c:if test="${sessionScope.previousRestaurants.size() > 0}">
    <a href="${pageContext.request.contextPath}/controller?command=all_restaurants&count_back=true&current_page=${sessionScope.currentPage}"
